...
230 tests. `pnpm test:dsp` runs the four above; `pnpm bench:fft` asserts throughput *and* flat heap growth over a minute of sustained analysis.
...
| Path | |
|---|---|
| `/[locale]/plat` | The plate. Spectrogram, waveform with the analysis-window bracket, hover readout, and the controls. |
| `/[locale]/susun` | Synthesis. Build a signal from sine components and watch the spectrum recover it, with the round-trip residual on screen. |
| `/[locale]/banding` | Our FFT against `AnalyserNode`, same input, per-bin difference and timings for both. |

Indonesian is the default locale; English is at `/en`. Signal-processing terms stay in English in both.
...
- The benchmark figures in this repository were measured on desktop. The mid-range-phone measurement has not been done yet.
